import postcss2, { Declaration } from "postcss";
import { h, b as addClassName$1, s as setInlineStyle$1 } from "./Code_BzKIzj_O.mjs";
import { Q as requireCssesc, S as getDefaultExportFromCjs } from "./astro/server_D8491iH5.mjs";
...
class TinyColor {
  ...
  toCmykString() {
    const { c, m: m2, y, k } = rgbToCmyk(this.r, this.g, this.b);
    return `cmyk(${c}, ${m2}, ${y}, ${k})`;
  }
  /**
  ...
  monochromatic(results = 6) {
    ...
    while (results--) {
      res.push(new TinyColor({ h: h2, s, v }));
      v = (v + modification) % 1;
    }
    return res;
  }
  ...
  polyad(n) {
    ...
    for (let i = 1; i < n; i++) {
      result.push(new TinyColor({ h: (h2 + i * increment) % 360, s: hsl.s, l: hsl.l }));
    }
    return result;
  }
  /**
...
function setProperty(node2, propertyName, value) {
...
function getClassNames(node2) {
  const stringOrArr = node2.properties?.className;
  if (!stringOrArr || stringOrArr === true)
    return [];
  if (Array.isArray(stringOrArr))
    return stringOrArr.map((className2) => className2.toString());
  return stringOrArr.toString().split(" ");
}
function addClassName(node2, className2) {
  const classNames = getClassNames(node2);
  if (classNames.indexOf(className2) === -1)
    classNames.push(className2);
  setProperty(node2, "className", classNames);
}
function getInlineStyles(node2) {
  const styles = /* @__PURE__ */ new Map();
  const styleString = node2.properties?.style?.toString().trim() || "";
  if (!styleString)
    return styles;
  const postCssOptions = { from: void 0 };
  try {
    const root2 = postcss2.parse(styleString, postCssOptions);
    root2.each((node22) => {
      if (node22.type === "decl")
        styles.set(node22.prop, node22.value);
    });
  } catch (error) {
  }
  return styles;
...
function setInlineStyles(node2, styles) {
  ...
  setProperty(node2, "style", styleString);
}
function setInlineStyle(node2, cssProperty, value, valueFormat = "raw") {
  const styles = getInlineStyles(node2);
  if (value !== null) {
    styles.set(cssProperty, valueFormat === "string" ? serializeCssStringValue(value) : value);
  } else {
    styles.delete(cssProperty);
  }
  setInlineStyles(node2, styles);
}
...
var InlineStyleAnnotation = class extends ExpressiveCodeAnnotation {
  ...
  render({ nodesToTransform, styleVariants }) {
    ...
    const isInlineStyleNode = (node2) => node2.tagName === "span" && // Our inline style nodes have no class names
    !getClassNames(node2).length && // Our inline style nodes contain CSS variable declarations
    node2.properties?.style?.toString().startsWith("--");
    const modifyExistingStyles = (node2, remove = false) => {
      const existingStyles = (node2.properties?.style?.toString() || "").split(";").map((style) => {
      ...
      if (modifiedStyles) {
        setProperty(node2, "style", modifiedStyles);
      } else if (node2.properties?.style) {
        delete node2.properties.style;
      }
    ...
    return nodesToTransform.map((node2) => {
      removeNestedConflictingStyles(node2);
      if (node2.type === "element" && isInlineStyleNode(node2)) {
        modifyExistingStyles(node2);
        return node2;
      }
      const transformedNode = h("span", { style: buildStyleString(newStyles) }, node2);
...
var preprocessor = postcss2([
  ...
  (root2) => {
    ...
    root2.walkRules((rule) => {
      if (rule.parent?.parent === root2) {
        rule.selectors = rule.selectors.map((selector2) => {
          if (selector2.indexOf(groupWrapperScope) === 0) {
            return selector2.slice(groupWrapperScope.length).trim() || "&";
          }
          return selector2;
        });
      }
...
var processor = postcss2([
  ...
  (root2) => {
    ...
    root2.walkRules((rule) => {
      rule.selectors = rule.selectors.map((selector2) => selector2.replace(regExpScopedTopLevel, "$1"));
    });
  },
  // Apply some simple minifications
  (root2) => {
    root2.raws.after = "";
    root2.walkComments((comment2) => {
      comment2.remove();
    });
    root2.walkRules((rule) => {
...
async function scopeAndMinifyNestedCss(css) {
  const postCssOptions = { from: void 0 };
  const root2 = postcss2.parse(`.${groupWrapperClassName}{${css}}`, postCssOptions);
  const preprocessedStyles = await preprocessor.process(root2, postCssOptions);
...
function isHastNode(node2) {
  return node2?.type ? typeof node2.type === "string" : false;
}
function isHastElement(node2) {
  return isHastNode(node2) && node2.type === "element";
}
...
function renderLineToAst({
  ...
  const renderedGutterElements = sortedGutterElements.map(({ pluginName, gutterElement }) => {
    try {
      const node2 = gutterElement.renderLine({ ...restContext, line, lineIndex });
      if (!isHastElement(node2))
        throw new Error(`renderLine function did not return a valid HAST Element node: ${JSON.stringify(node2)}`);
      return node2;
    } catch (error) {
      const msg = error instanceof Error ? error.message : error;
...
function getRenderEmptyLineFn(context) {
  return () => {
    ...
    const renderedGutterElements = sortedGutterElements.map(({ pluginName, gutterElement }) => {
      try {
        const node2 = gutterElement.renderPlaceholder();
        if (!isHastElement(node2))
          throw new Error(`renderPlaceholder function did not return a valid HAST Element node: ${JSON.stringify(node2)}`);
        return node2;
      } catch (error) {
        const msg = error instanceof Error ? error.message : error;
...
function validateAnnotationRenderOutput(nodes, expectedLength) {
  ...
  nodes.forEach((node2, nodeIndex) => {
    if (!node2 || !node2.type)
      throw new Error(`Annotation render function returned an invalid node at index ${nodeIndex}: ${JSON.stringify(node2)}`);
  });
}
...
function parseCssValue(value, min, max, valueFor100Percent) {
  const isPercentage2 = value.endsWith("%");
  const floatValue = parseFloat(value);
  const convertedValue = isPercentage2 ? floatValue * (valueFor100Percent ?? max) / 100 : floatValue;
...
var IndentAnnotation = class extends ExpressiveCodeAnnotation {
  render({ nodesToTransform }) {
    return nodesToTransform.map((node2) => h("span.indent", node2));
  }
};
var ExpressiveCodeEngine = class {
  ...
  async getThemeStyles() {
    ...
    ].filter((selector2) => selector2).join(",");
  ...
  "../../../node_modules/.pnpm/parse-numeric-range@1.3.0/node_modules/parse-numeric-range/index.js"(exports, module) {
    function parsePart(string2) {
      ...
      for (let str of string2.split(",").map((str2) => str2.trim())) {
        ...
        } else if (m2 = str.match(/^(-?\d+)(-|\.\.\.?|\u2025|\u2026|\u22EF)(-?\d+)$/)) {
          let [_, lhs, sep, rhs] = m2;
          if (lhs && rhs) {
            lhs = parseInt(lhs);
            rhs = parseInt(rhs);
...
var TextMarkerAnnotation = class extends ExpressiveCodeAnnotation {
  ...
  renderFullLineMarker({ nodesToTransform }) {
    return nodesToTransform.map((node2) => {
      if (node2.type === "element") {
        addClassName$1(node2, "highlight");
        addClassName$1(node2, this.markerType);
        if (this.label) {
          addClassName$1(node2, "tm-label");
          setInlineStyle$1(node2, "--tmLabel", this.label, "string");
        }
      }
      return node2;
    });
  ...
  renderInlineMarker({ nodesToTransform }) {
    return nodesToTransform.map((node2, idx) => {
      const transformedNode = h(this.markerType, node2);
...
export {
  ...
  setLuminance as B,
  stableStringify as C,
  toHexColor as D,
  ExpressiveCodeTheme as E,
  validateExpressiveCodeAnnotation as F,
  LanguagesWithFencedFrontmatter as G,
  InlineStyleAnnotation as I,
  LanguageGroups as L,
  MetaOptions as M,
  PluginStyleSettings as P,
  ExpressiveCodeEngine as a,
  pluginTextMarkers as b,
  pluginFrames as c,
  ...
  codeLineClass as l,
  darken as m,
  ensureColorContrastOnBackground as n,
  getCssVarName as o,
  pluginFramesTexts as p,
  getFirstStaticColor as q,
  getLuminance as r,
  getStaticBackgroundColor as s,
  isInlineStyleAnnotation as t,
  lighten as u,
  multiplyAlpha as v,
  onBackground as w,
  resolveStyleVariants as x,
  runHooks as y,
  setAlpha as z
};
